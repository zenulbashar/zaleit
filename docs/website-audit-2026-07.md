...
> **Note for this branch:** pushing `claude/zale-website-audit-zlywun` will trigger this workflow and fast-forward `main`. This audit adds only documentation under `docs/`, so the production impact is a new unlinked Markdown file — but the mechanism is exactly the finding, and it is worth watching the run.
